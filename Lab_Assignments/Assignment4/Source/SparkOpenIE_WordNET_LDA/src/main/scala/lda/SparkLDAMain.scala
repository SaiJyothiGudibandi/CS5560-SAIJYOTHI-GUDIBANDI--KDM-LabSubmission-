...
object SparkLDAMain {
  ...
  private def run(params: Params) {
    ...
    val (corpus, vocabArray, actualNumTokens) =
      preprocess(sc, params.input)                      //params.input: folder name
    corpus.cache()
    ...
    // Run LDA.
    val lda = new LDA()             //imp:creating lda
  ...
  private def preprocess(sc: SparkContext,paths: Seq[String]): (RDD[(Long, Vector)], Array[String], Long) = {
    ...
    //Reading Stop Words
    val stopWords=sc.textFile("data/stopwords.txt").collect()       //making stop word as broadcast variables
    ...
    val stopWordRemovedDF=df.map(f=>{                           //
      ...
      val filteredF=f.map(_.replaceAll("[^a-zA-Z]"," "))      //removing other than letters; map is scala
        //Filter out the Stop Words
        .filter(ff=>{                                         //with in map
